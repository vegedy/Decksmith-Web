  ...
  if (process.argv.includes('--accessibility-only')) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (await code.count()) {
        assert.ok(
          (
            await code.evaluate((el) => getComputedStyle(el).fontFamily)
          ).includes(deck.display.fonts.mono),
        )
        assert.equal(
          await page.evaluate(
      ...
      assert.deepEqual(
        overflow,
        [],
        `Content overlaps footer on slide ${i + 1}`,
      )
      ...
      const bounds = await frame.evaluate((el) => {
        ...
        return [
          ...el.querySelectorAll(
            '.equation, .katex-html, .notation-table, .source-footer, .scientific-code, .scientific-figure, .callout, .takeaway, .visualization, .interactive-reveal, .references-list',
          ),
        ]
          .filter((node) => {
    ...
    if (!process.env.SMOKE_URL) {
      ...
      for (const frame of await page.locator('.deck-frame').all()) {
        const overflow = await frame.evaluate((el) => {
          const footer = el
            .querySelector('.deck-footer')!
            .getBoundingClientRect()
          const area = el.getBoundingClientRect()
          return [
            ...el.querySelectorAll(
              'h1, p, li, .equation, .katex-html, .notation-table, .source-footer, .scientific-code, .scientific-figure, .callout, .takeaway, .visualization, .interactive-reveal',
            ),
          ]
            .filter((node) => {
              ...
              return (
                r.width &&
                (r.bottom > footer.top + 1 || r.right > area.right + 1)
              )
            })
